...
AUCTION_MAP_FILE = "auction_provenance_map_visual_table_portrait_refined_grouped_by_provenance.png"
...
def show_image_or_warning(image_path: Path, caption: str | None = None):
    if image_path.exists():
        st.image(Image.open(image_path), caption=caption, use_container_width=True)
    else:
        st.warning(f"Missing visual: {image_path.name}")
        st.write("Files Streamlit can see:")
        st.write(list(Path(".").rglob("*")))
...
"""
    ...
    auction_map_path = VISUALS_DIR / AUCTION_MAP_FILE

    show_image_or_warning(
        auction_map_path,
        caption="Auction Provenance Map — grouped by provenance type, with Estate first."
    )
